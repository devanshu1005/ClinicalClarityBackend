...
const validateDoctorClinicSchedules = require('../../utils/validateDoctorClinicSchedules');
...
const createDoctor = async (req, res, next) => {
  try {
    ...
    if (!Array.isArray(clinics) || clinics.length === 0) {
      return res.status(400).json({
        success: false,
        message: "clinics must be a non-empty array",
      });
    }

    const clinicIds = clinics.map(({ clinicId }) => clinicId);
    ...
    const existingClinics =
      await doctorService.validateClinicIds(clinicIds);

    if (existingClinics.length !== clinicIds.length) {
      return res.status(400).json({
        success: false,
    ...
    const validation =
      validateDoctorClinicSchedules(clinics);
